...
try:
    ...
    cur.execute("create table dashboard(Book_ID primary key,subject text, author text, status text)")
...
def acn_create_frm():
        ...
        def open_account(): #method for account openning
            messagebox.showinfo("Account Create","Account Created Successfully")
            ...
            con = sqlite3.connect(database="library.sqlite")
            cur = con.cursor()
            cur.execute("insert into users (first_name, last_name, email, contact_no, password,joindate) values (?,?,?,?,?,?)",(name_f,name_l,email,contact_num,pwd,open_date))
            con.commit()
            con.close()
...
def open_acn_screen():
        ...
        def login_screen():
            ...
            con.close()
            if(row == None):
                messagebox.showerror("Login","Invalid Email ID or Password")
            else:
                frm.destroy()
                screen_login()
...
def screen_login():
    ...
    def book_add():
        ...
        lbl_subject.place(relx=.4, rely=.25)

        subject_entry = Entry(win, bg="white", width=20, fg="black", font=("ariel", 20, "bold"))
        subject_entry.place(relx=.51, rely=.25)

         #login left for combobox
    ...
    def book_withdraw():
        ...
        std_dept_entry = Entry(win, bg="white", width=20, fg="black", font=("ariel", 20, "bold"))  #add combobox
        std_dept_entry.place(relx=.44, rely=.31)

        book_name_lbl = Label(win, text="Book Name :-", bg="#F8E8EE", fg="black", font=('Chalkboard', 30, 'italic'))
        book_name_lbl.place(relx=.3, rely=.4)
        book_name_entry = Entry(win, bg="white", width=20, fg="black", font=("ariel", 20, "bold"))
        book_name_entry.place(relx=.44, rely=.41)
        ...
        issue_btn = Button(frm, text="Issue", bg="powder blue", fg="black", font=("Monaco", 20, 'bold'))
        issue_btn.pack()
        issue_btn.place(relx=.4, rely=.75)
    ...
    def booksearch():
        ...
        book_name = Label(win, text="Book Name :-", bg="#F8E8EE", fg="black", font=("Chalkboard", 20, 'bold'))
        ...
        search_btn = Button(frm, text="Search", bg="powder blue", fg="black", font=("Monaco", 20, 'bold'))
        search_btn.pack()
    ...
    def send_otp():
        ...
        if(row == None):
            ...
            if(row[0]==email):
                ...
                gen_otp_btn = Button(win, text="Get OTP", bg="powder blue", fg="black",command=otp_get,font=("Monaco", 20, 'bold'))
                gen_otp_btn.pack()
